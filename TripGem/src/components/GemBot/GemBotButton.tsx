'use client'

import { useGemBotStore } from '@/stores/gembot'
import { GemBotOverlay } from './GemBotOverlay'

export function GemBotButton() {
  const { isOpen, toggle } = useGemBotStore()

  return (
    <>
      <button
        onClick={toggle}
        className="fixed sm:bottom-6 bottom-[72px] left-3 z-[10001] hidden sm:flex size-12 items-center justify-center rounded-full border-2 border-[var(--accent)]/50 bg-[var(--bg3)] backdrop-blur-sm shadow-lg shadow-[var(--accent)]/20 transition-all hover:scale-110 hover:border-[var(--accent)] hover:shadow-[var(--accent)]/40 active:scale-95"
        style={{
          boxShadow: isOpen
            ? '0 0 20px var(--accent), 0 0 8px var(--accent)'
            : '0 0 10px rgba(var(--accent-rgb), 0.4)',
          animation: 'gembot-pulse 2s ease-in-out infinite',
        }}
        aria-label={isOpen ? 'Close GemBot' : 'Open GemBot'}
        aria-expanded={isOpen}
      >
        <img
          src="/og-image.gif"
          alt="TripGem"
          className="size-7 rounded-md"
          style={{ filter: 'drop-shadow(0 0 4px rgba(var(--accent-rgb), 0.5))' }}
        />
      </button>
      <GemBotOverlay />
    </>
  )
}
