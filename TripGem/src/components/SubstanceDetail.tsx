...
const ScoreBreakdownPopup = dynamic(
  () => import('@/components/ScoreBreakdownPopup'),
  { ssr: false, loading: () => null }
)
...
export default function SubstanceDetail({ substance, comboMatrix, relatedSubstances, allSubstances }: SubstanceDetailProps) {
    ...
    <div className="flex flex-col flex-1 min-h-0 w-full mx-auto max-w-[1800px]">
      ...
      >
        <div className="flex items-center justify-around h-16">
          <Link
            href="/"
            className="flex flex-col items-center justify-center gap-1 px-4 py-2 rounded-lg transition-all duration-300 text-[var(--text3)]"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="m2.25 12 8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25" />
            </svg>
            <span className="text-[10px] font-mono font-semibold">Home</span>
          </Link>
          <Link
            href="/"
            className="flex flex-col items-center justify-center gap-1 px-4 py-2 rounded-lg transition-all duration-300 text-[var(--text3)]"
          >
            ...
            <span className="text-[10px] font-mono font-semibold">Search</span>
          </Link>
        </div>
      </nav>
  ...
  <p className="text-xs text-[var(--text3)] max-w-md mx-auto leading-relaxed">
      ...
      {scoreBreakdownOpen && (
        <ScoreBreakdownPopup substances={allSubstances} />
      )}
    </div>
  )
}
