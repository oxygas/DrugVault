  ...
  return (
    <div className="w-full space-y-10 sm:space-y-14">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
        {/* Body Weight Card */}
        <div
          className="rounded-2xl border border-[var(--border)] p-8 sm:p-10 space-y-6"
          style={{ background: 'rgba(8,8,24,0.4)' }}
        >
          <label className="flex items-center gap-4 text-sm font-mono text-[var(--text4)] uppercase tracking-[0.2em]">
        ...
        {/* Dosage + Form Card */}
        <div
          className="rounded-2xl border border-[var(--border)] p-8 sm:p-10 space-y-8"
          style={{ background: 'rgba(8,8,24,0.4)' }}
        >
          <div className="space-y-6">
            ...
            <input
              ...
              placeholder="e.g. 300"
              className="w-full px-8 py-5 rounded-xl text-xl text-white placeholder:text-[var(--text4)] bg-[rgba(10,10,30,0.5)] border border-[var(--border2)] focus:outline-none focus:border-blue-500/50 focus:ring-2 focus:ring-blue-500/20 transition-all"
            />
          </div>
            ...
            <div className="flex rounded-xl overflow-hidden border-2 border-[var(--border2)] bg-[rgba(10,10,30,0.5)]">
              {(['hbr', 'freebase'] as const).map(f => (
                <button
                  key={f}
                  onClick={() => setForm(f)}
                  className={`flex-1 py-5 text-lg font-bold tracking-wider transition-all ${
                    form === f
      ...
      {isWaiting && (
        <div
          className="rounded-2xl border border-[var(--border)] py-20 px-10 text-center"
          style={{ background: 'rgba(8,8,24,0.3)' }}
        >
          <div className="w-20 h-20 mx-auto mb-8 rounded-2xl bg-blue-500/10 border border-blue-500/20 flex items-center justify-center">
            <svg className="w-10 h-10 text-blue-400" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 3v2.25m6.364.386l-1.591 1.591M21 12h-2.25m-.386 6.364l-1.591-1.591M12 18.75V21m-4.773-4.227l-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0z" />
            </svg>
          </div>
          <p className="text-xl text-[var(--text3)] leading-relaxed">Enter your body weight to calculate the plateau.</p>
        </div>
      )}

      {/* Results */}
      {result && !isWaiting && (
        <div className="space-y-8 sm:space-y-10" style={{ animation: 'fadeInUp 0.5s cubic-bezier(0.16,1,0.3,1) both' }}>
          {/* Stats */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-5 sm:gap-6">
            {[
              ...
              <div
                key={stat.label}
                className="rounded-xl border border-[var(--border)] p-6 sm:p-8"
                style={{ background: 'rgba(8,8,24,0.3)' }}
              >
                <div className="text-xs font-mono text-[var(--text4)] uppercase tracking-[0.15em] mb-3">{stat.label}</div>
          ...
          {result.type === 'below' && (
            <div
              className="rounded-2xl border-2 p-10 sm:p-12 flex items-start gap-8"
            ...
            >
              ...
              <div>
                <h4 className="text-2xl font-display font-bold text-amber-300 mb-3">Below First Plateau</h4>
                <p className="text-lg text-[var(--text3)] leading-relaxed">
                  ...
                  Take approximately <strong className="text-amber-300 text-xl">{result.needMore.toFixed(0)} mg</strong> more
          ...
          {result.plateau && (
            <div
              className="rounded-2xl border-2 p-10 sm:p-14 space-y-10"
              style={{ background: `${result.plateau.color}05`, borderColor: `${result.plateau.color}22` }}
            >
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-6">
                <div>
                  <div className="text-xs font-mono text-[var(--text4)] uppercase tracking-[0.2em] mb-4">Current Plateau</div>
                  <h2
                    className="text-4xl sm:text-5xl lg:text-6xl font-display font-extrabold leading-tight"
                    style={{ color: result.plateau.color }}
                  >
                    {result.plateau.name}
                  </h2>
                  <p className="text-base text-[var(--text4)] font-mono mt-3">{result.plateau.range}</p>
                </div>
                <span
                  className="px-8 py-3 rounded-full text-base font-mono font-bold uppercase tracking-wider flex-shrink-0 self-start"
              ...
              <div>
                <h4 className="text-sm font-mono text-[var(--text4)] uppercase tracking-[0.2em] mb-5">Expected Effects</h4>
                <p className="text-lg sm:text-xl text-[var(--text3)] leading-relaxed">{result.plateau.effects}</p>
              </div>

              <div>
                <h4 className="text-sm font-mono text-[var(--text4)] uppercase tracking-[0.2em] mb-5">Risks & Side Effects</h4>
              ...
              {result.plateau.level >= 3 && (
                ...
                >
                  <svg className="w-8 h-8 mt-0.5 flex-shrink-0" style={{ color: result.plateau.color }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" />
                  </svg>
                  <div>
                    <strong className="text-xl" style={{ color: result.plateau.color }}>High-dose warning</strong>
                    <p className="text-lg text-[var(--text3)] leading-relaxed mt-2">
      ...
      {!result && !isWaiting && (weight || dosage) && (
        <div
          ...
          style={{ background: 'rgba(8,8,24,0.3)' }}
        >
          <div className="w-20 h-20 mx-auto mb-8 rounded-2xl bg-[var(--accent)]/10 border border-[var(--accent)]/20 flex items-center justify-center">
            <svg className="w-10 h-10 text-[var(--accent)]" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 3v2.25m6.364.386l-1.591 1.591M21 12h-2.25m-.386 6.364l-1.591-1.591M12 18.75V21m-4.773-4.227l-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0z" />
            </svg>
          </div>
          <p className="text-xl text-[var(--text3)] leading-relaxed">Enter both weight and dosage to calculate your plateau.</p>
        </div>
      )}
      ...
      {!result && !weight && !dosage && (
        <div
          className="rounded-2xl border border-[var(--border)] py-28 px-10 text-center"
          style={{ background: 'rgba(8,8,24,0.2)' }}
        >
          <div className="w-24 h-24 mx-auto mb-10 rounded-3xl bg-[var(--accent)]/5 border border-[var(--border)] flex items-center justify-center">
            <svg className="w-12 h-12 text-[var(--text4)]" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M9.75 3.104v5.714a2.25 2.25 0 01-.659 1.591L5 14.5M9.75 3.104c-.251.023-.501.05-.75.082m.75-.082a24.301 24.301 0 014.5 0m0 0v5.714c0 .597.237 1.17.659 1.591L19.8 15.3M14.25 3.104c.251.023.501.05.75.082M19.8 15.3l-1.57.393A9.065 9.065 0 0112 15a9.065 9.065 0 00-6.23.693L5 14.5m14.8.8l1.402 1.402c1.232 1.232.65 3.318-1.067 3.611A48.309 48.309 0 0112 21c-2.773 0-5.491-.235-8.135-.687-1.718-.293-2.3-2.379-1.067-3.61L5 14.5" />
            </svg>
          </div>
          <h3 className="text-3xl font-display font-bold text-white mb-4">DXM Plateau Calculator</h3>
          <p className="text-lg text-[var(--text4)] max-w-lg mx-auto leading-relaxed">
