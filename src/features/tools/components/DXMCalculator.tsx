    ...
    <div className="w-full max-w-4xl mx-auto">
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-purple-500/50 focus:ring-2 focus:ring-purple-500/20 transition-all"
              />
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div className="bg-white/5 border border-white/10 rounded-xl p-4">
              <div className="text-xs text-gray-400 mb-1">HBr Equivalent</div>
              <div className="text-2xl font-bold text-white">{result.hbrDosage.toFixed(0)} <span className="text-sm font-normal text-gray-400">mg</span></div>
            </div>
            <div className="bg-white/5 border border-white/10 rounded-xl p-4">
              <div className="text-xs text-gray-400 mb-1">mg / kg</div>
              <div className="text-2xl font-bold text-white">{result.mgPerKg.toFixed(2)}</div>
            </div>
            <div className="bg-white/5 border border-white/10 rounded-xl p-4">
              <div className="text-xs text-gray-400 mb-1">Weight</div>
              <div className="text-2xl font-bold text-white">{result.weightKg.toFixed(1)} <span className="text-sm font-normal text-gray-400">kg</span></div>
            </div>
            <div className="bg-white/5 border border-white/10 rounded-xl p-4">
              <div className="text-xs text-gray-400 mb-1">Form</div>
              <div className="text-2xl font-bold text-white">{form === 'hbr' ? 'HBr' : 'Freebase'}</div>
            </div>
          </div>
          ...
          {result.type === 'below' && (
            <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4 mb-6 flex items-start gap-3">
              ...
              <div>
                <div className="text-amber-400 font-medium mb-1">Below threshold</div>
                <p className="text-sm text-gray-300">
                  This dose is below the first plateau. Add approximately <strong className="text-white">{result.needMore.toFixed(0)} mg</strong> more to reach the first plateau.
                </p>
              </div>
            </div>
          )}
          ...
          {result.type === 'plateau' && (
            <>
              <div className="bg-white/5 border border-white/10 rounded-2xl p-6 mb-6">
                <div className="relative h-4 rounded-full overflow-hidden mb-4" style={{ background: 'rgba(255,255,255,0.05)' }}>
                  <div
                ...
                <div className="space-y-4">
                  ...
                  >
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <h3 className="text-2xl font-bold text-white">{result.plateau.name}</h3>
                        <p className="text-sm text-gray-400 mt-1">{result.plateau.range}</p>
                      </div>
                      <span
                    ...
                    <p className="text-gray-300 mb-4">{result.plateau.effects}</p>
                    <div>
                      <div className="text-sm font-medium text-gray-400 mb-2">Effects & Risks:</div>
                  ...
                  {result.plateau.level >= 3 && (
                    <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-4 flex items-start gap-3">
                      ...
                      <div>
                        <div className="text-red-400 font-bold mb-1">High-Dose Warning</div>
                        <p className="text-sm text-gray-300">
      ...
      {!result && !weight && !dosage && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-400">Enter your weight and dosage to calculate</p>
        </div>
      )}
    </div>
  )
